  ...
  describe "testa Algoritmos Matrizes Lista" $ do
    ...
    describe "testa funções extras" $ do
      ...
      it "deve retornar uma matriz negada" $ do
        ...
        let naxs = valores na
        let allN =
              foldr (\x _ -> if x < 0 then True else False) False $ concat naxs
        allN `shouldBe` True
        axs `shouldSatisfy` (/= naxs)

      it
          "deve retornar uma matriz vazia de signum caso a matriz não seja quadrada"
        $ do
      ...
      it "deve retiornar uma matriz identidade" $ do
        ...
        let calcOne =
              foldr (\x acc -> if x == 1 then acc + 1 else acc) 0 $ concat ys :: Integer
        xs `shouldSatisfy` (/= ys)
    ...
    describe "testa a soma de matrizes lista" $ do
      it
          "deve retornar uma matriz vazia caso as linhas de A e B sejam diferentes"
        $ do
            let a  = matriz 2 2 :: (Matriz Int)
      ...
      it
          "deve retornar uma matriz vazia caso as colunas de A e B sejam diferentes"
        $ do
            let a  = matriz 2 2 :: (Matriz Int)
    ...
    describe "testa a multiplicação de matrizes lista" $ do
      it
          "deve retornar uma matriz vazia caso as colunas de A sejam diferentes das linhas de B"
        $ do
            let a  = matriz 2 2 :: (Matriz Int)
            let b  = matriz 3 2 :: (Matriz Int)
            let nA = colunas a
      ...
      it
          "deve retornar uma matriz vazia caso as linhas de B sejam diferentes das colunas de A"
        $ do
            let a  = matriz 3 3 :: (Matriz Int)
            let b  = matriz 2 3 :: (Matriz Int)
            let nA = colunas a
