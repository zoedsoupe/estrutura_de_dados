...
insereItem x a@(Conjunto ys) | not $ pertence x a = Conjunto (x : ys)
                             | otherwise          = a
...
contem (Conjunto (x : xs)) b@(Conjunto _) =
  pertence x b && contem (Conjunto xs) b
...
-- | Funções de ajuda
nub :: Eq a => [a] -> [a]
nub = foldl (\seen x -> if elem x seen then seen else seen ++ [x]) []
