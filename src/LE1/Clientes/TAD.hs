...
criaCliente
  :: (Integer, String, String, String, String, String, Decimal) -> Cliente
...
salvaClientes xs path =
  mapM_ (\x -> salvaCliente x path) xs >> putStrLn "Os Clientes foram salvos!"
...
leCliente (x : _) | x == B.empty                  = Invalido
leCliente (c : n : t : e : dt_p : dt_u : v_u : _) = Cliente cod
                                                            n
                                                            e
                                                            t
                                                            dt_p
...
clientes :: Clientes
clientes =
  [ Cliente { codigo              = 385812323496637
            , nome                = B.pack "Holie"
